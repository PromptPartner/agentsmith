...
reconciliation, doctor output, compatibility reporting, helpers, secret checks, and hook handling.
...
- `--with-hooks` installs the Python secret gate as a git pre-commit hook when no foreign hook would
  be overwritten.
- `--safety cautious|trusted` maps approval and sandbox settings for native clients.
...
| `agentsmith secret-scan` | Detect common live-secret shapes in project files. |
| `agentsmith doctor` | Report each capability independently for selected agents. |
...
CI runs those checks natively on Ubuntu, macOS, and Windows. Existing POSIX-only guardrail tests
remain a separate Ubuntu job; they do not stand in for Windows runtime evidence.
